...
public class RechargeVo extends  DataEntity<RechargeVo>{
	...
	private String rechargeType;    //`rechargeType` char(2) DEFAULT NULL COMMENT '充值类型(0-银行转账，1-第三方支付)',
	...
	private String createTime;    //`createTime` datetime DEFAULT NULL COMMENT '充值时间',
	...
	@ExcelField(title="用户手机号",type=1,align=2,sort=1)
	...
	@ExcelField(title="充值金额",type=1,align=2,sort=3)
	...
	@ExcelField(title="手续费（元）",type=1,align=2,sort=4)
	...
	@ExcelField(title="手续费承担方类型",type=1,align=2,sort=5)
	public String getFeesBearer() {
	    if (!"".equals(feesBearer)) {
            return feesBearer;
        }
		return "交易方承担";
	}
	...
	@ExcelField(title="平台流水号",type=1,align=2,sort=6)
	...
	@ExcelField(title="充值时间",type=1,align=2,sort=2)
