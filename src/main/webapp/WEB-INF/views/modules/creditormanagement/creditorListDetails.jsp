                ...
                <div class="tableBox">
					<table style="margin-left: 80px;width: 850px;font-size: 14px;">
						<tr>
							...
							<td style="width: 200px;">${creditorDeails.assetCode}</td>
							<td style="width: 150px;padding-left: 50px;" align="right">债权名称:</td>
							<td>${creditorDeails.borrowtitle}</td>
						</tr>
						<tr>
							<td  align="right">债权价值:</td><td>${creditorDeails.amount}元</td>
							<td style="width: 150px;padding-left: 50px;" align="right">出借年化利率:</td>
							<td>
							${creditorDeails.capitalRate}%
							</td>
						</tr>
						<tr>
							<td align="right" >最低出借金额:</td>
							<td>${creditorDeails.mintendersum}元</td>
							<td align="right" >还款方式:</td>
							<td> ${fns:getDictLabel(creditorDeails.payMethod, 'pay_method', '')}</td>
						</tr>
						<tr>
							...
							<td style="width: 350px;">
							  		${creditorDeails.surplusDeadlline}/${creditorDeails.deadline}
							</td>
						</tr>
						<tr>
							<td align="right">状态:</td>
							<td>${fns:getDictLabel(creditorDeails.borrowstatus, 'BORROWSTATUS', '')}</td>
						</tr>
					</table>

                </div><!--NewListConfig--><!--左右-->
				<form id="searchForm" action="${ctx}/creditormanagement/creditor/creditorListDetails">
				    <input id="id" name="id" type="hidden" value="${creditorDeails.id}"/>
				    <input id="tinvestId" name="tinvestId" type="hidden" value="${creditorDeails.tinvestId}"/>
				    <input id="loannumber" name="loannumber" type="hidden" value="${creditorDeails.loannumber}"/>
                                ...
                                <div class="tableBox">
										<table style="margin-left: 80px;width: 850px;font-size: 14px;">
											<tr>
												...
												<td style="width: 200px;">${fn:substring(creditorDeails.borrowRealname,0,1)}**</td>
												<td style="width: 150px;padding-left: 50px;" align="right">所在城市</td>
												<td>${creditorDeails.cityCode}</td>
												<td style="width: 222px;padding-left: 50px;" align="right">身份证号</td>
												<td style="width: 222px;padding-left: 50px;" align="left">${fn:substring(creditorDeails.idcard,0,3)}******</td>
											</tr>
											<tr>
												...
												<td>
												<c:if test="${creditorDeails.sex=='0'}">女</c:if>
												<c:if test="${creditorDeails.sex=='1'}">男</c:if>
												</td>
											</tr>
										</table>
                                </div>
                                ...
                                <div class="tableBox">
										<table style="margin-left: 80px;width: 850px;font-size: 14px;">
											<tr>
												...
												<td style="width: 200px;">${creditorDeails.loannumber}</td>
												<td style="width: 150px;padding-left: 50px;" align="right">借款用途</td>
												<td>${creditorDeails.usageOfLoan}</td>
											</tr>
											<tr>
												<td  align="right">借款期限</td>
												<td>${creditorDeails.deadline}个月</td>
												<td style="width: 150px;padding-left: 50px;" align="right">还款方式</td>
												<td>
											${fns:getDictLabel(creditorDeails.payMethod, 'pay_method', '')}
												</td>
											</tr>
											<tr>
												<td  align="right">剩余期限</td>
												<td>${creditorDeails.surplusDeadlline}个月</td>
                                        	...
                                        	<c:when test="${not empty transfers.list}">
												<c:forEach items="${transfers.list}" var="inv" varStatus="index2">
													<tr>
														...
														<td><a href="${ctx}/creditormanagement/creditor/repayBillplan?investId=${inv.investId}">回款计划</a></td>
													</tr>
												</c:forEach>
                                       		</c:when>
										<c:otherwise>
