...
<body>
    ...
    <form:form id="searchForm" modelAttribute="coupon" action="${ctx}/coupon/couponManage/couponAuditList" method="post" class="breadcrumb form-search">
        ...
        <ul class="ul-form">
            ...
            <li><label>优惠券状态：</label>
                ...
                <form:select path="status" class="input-medium">
                    <form:option value="" label="全部"/>
                    <form:options items="${fns:getDictList('couponAuthstatus')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
                </form:select>
            </li>
            <li><label>创建时间：</label>
        ...
        <c:forEach items="${page.list}" var="coupon" varStatus="tfdAcc">
            <tr>
              <td>${tfdAcc.count+page.pageSize*(page.pageNo-1)}</td> 
                ...
                <td>${fns:getDictLabel(coupon.status,'couponstatus','')}</td>
                <td><fmt:formatDate value="${coupon.createDate}" pattern="yyyy-MM-dd HH:mm"/></td>
                <td><fmt:formatDate value="${coupon.auditTime}" pattern="yyyy-MM-dd HH:mm"/></td>
                <td>
                    <%-- <a href="${ctx}/coupon/couponManage/deleteCoupon?id=${coupon.id}" onclick="return confirmx('确认要删除该平台账户吗？', this.href)">删除</a>
                    <a href="${ctx}/coupon/couponManage/toSaveCoupon?id=${coupon.id}">修改</a> --%>
                    <a href="${ctx}/coupon/couponManage/couponAuditDetail?id=${coupon.id}">查看</a>
                </td>
            </tr>
        </c:forEach>
        </tbody>
