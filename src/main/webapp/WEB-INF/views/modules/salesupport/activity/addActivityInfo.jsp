...
<head>
	...
	<script type="text/javascript">
		...
		/* 	$("#btnOk").click(function(){
				...
				$.ajax({  
					url : ctx+'/activity/activityInfo/addCouponList',
					type : 'post',
					dataType: "json",
				    data: {
		                 "ids": idstr,
					...
					success : function(data) {
						for(var i=0;i<data.length;i++) {
					   		 ...
					   		 $("#sourceTable").append( "<tr  align='center' style='text-align: center;'>");
							 $("#sourceTable").append("<td>"+data[i].couponTypeId+"  "+data[i].faceValue+" "+data[i].effectiveDays+" "+data[i].loanAmountMin+"  "+data[i].loanTermList+"</td>");
							 $("#sourceTable").append("<td><input style='font-size: 20px; color: blue;width: 100px;' type='button' value='—'  onclick='deleteRow(this,'sourceTable')'></td>");         
							 $("#sourceTable").append("</tr><br/>");
...
<body>
	...
	<li class="active"><a href="javascript:;">活动新增</a></li>
	...
	<form:form id="inputForm" modelAttribute="activityInfo" action="${ctx}/activity/activityInfo/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			<div class="controls" style="font-size: 18px; color: red;">
				${isMessage}
			</div>
		</div>
...
<%-- 					<form:option value="" label=""/> --%>
		...
		<div class="control-group">
			...
			readonly="readonly" maxlength="20" class="input-medium Wdate" value=""
			 onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:true,minDate:'${currenDate}'});">
		...
		<div class="control-group">
			...
			readonly="readonly" maxlength="20" class="input-medium Wdate" value=""
			 onclick="WdatePicker({dateFmt:'yyyy-MM-dd 23:59:59',isShowClear:true,minDate:'${currenDate}'});">  
			 
		</div>
		</div>

		    <div class="control-group"  id="imgUrl"  style="display: none;">
			<label class="control-label">活动图片：</label>
			<div class="controls">
			
			<input type="hidden" readonly="readonly" id="imageurl" name="imageurl" value="${activityInfo.imageurl}" />
		...
		<div class="control-group" id="hrefUrl" style="display: none;">
...
<div class="control-group">
		...
		<tbody>
		<c:forEach items="${couponList}" var="coupon" varStatus="tfdAcc">
			<tr align="center">
