				...
				<form action="" method="post" id="releaseBorrowForm">
									...
									<c:when test="${not empty borrows}">
										<c:forEach items="${borrows}" var="vo" varStatus="status">
											<tr>
												...
												<td>
													<a href="javascript:void(0);" onclick="deleteBorrow(this,'${vo.borrowId}');">删除</a> 
					...
					<h2 class="H2 mt26">发布设置</h2>
						<div class="left">
							...
							<textarea rows="4" cols="40" id="warnings" name="riskWarning" <c:if test="${''==borrows[0].riskWarning}">placeholder = "市场有风险 出借须谨慎"</c:if>
						...
						<div class="left">
							...
							<textarea rows="4" cols="40" id="usageofloan" name="usageofloan"   <c:if test="${''==borrows[0].projectBrief}">placeholder = "请填写项目介绍"</c:if>
								onkeyup="if($(this).val().length > 1000) $(this).val($(this).val().substring(0,1000));" maxlength="1000"><%-- <c:if test="${''==borrows[0].usageofloan}">请填写项目介绍</c:if> --%>${borrows[0].projectBrief}</textarea>
						...
						<div class="left">
							...
							<input type="text" class="NewListConfig1Text" id="raiseterm" name="raiseterm" value="${borrows[0].raiseterm}" onkeyup="checkRaiseterm();"/>
							<select class="" style="width:160px;height: 40px" id="raisetermunit" name="raisetermunit" onchange="checkRaiseterm();">
			                   	<option value="1" <c:if test="${borrows[0].raisetermunit==1}">selected</c:if> >天</option>
			                    <%-- <option value="0" <c:if test="${borrows[0].raisetermunit==1}">selected</c:if>>小时</option> --%>
			                </select>
					...
					<div class="Mbtm wd2" align="center">
						...
						<a href="Njavascript:void(0);" class="Mbtn2 Mbtn2Color2" style="line-height: 32px" onclick="history.go(-1);">上一步</a>
					</div>
				</form>
			</div>
